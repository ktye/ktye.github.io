"use strict";let cnvplot=(cnv,p,/*,caption,cnv,sld,det,txt,cap,*/...a)=>{let c=cnv.getContext("2d",{willReadFrequently:true}),id=cnv.id,w=cnv.width,h=cnv.height,single=0;
 ...
 let font1,font2,ufont1,ufont2,cols=0,equal,resize=0,stati=0,hi=[],sld,cap,tab,onzoom,onpan,onmeasure; //args, e.g. "font1","12pt monospace",.. hi=[{plot-idx,line-idx,point-idx||-1}]
 for(let i=0;i<a.length;i++){let x=a[i];x=="font1"?(ufont1=a[++i]):x=="font2"?(ufont2=a[++i]):x=="cols"?(cols=a[++i]):x=="equal"?(equal=1):x=="resize"?(resize=1):x=="static"?(stati=1):x=="slider"?(sld=a[++i]):x=="caption"?cap=a[++i]:x=="table"?tab=a[++i]:x=="onzoom"?(onzoom=a[++i]):x=="onpan"?(onpan=a[++i]):x=="onmeasure"?(onmeasure=a[++i]):0}
 ...
 let eqlimits=p=>{let P=p.filter((_,i)=>!usrlimits[i]);if(P.length<2)return;let r=0,x0=Infinity,x1=-x0,y0=Infinity,y1=-y0;
  P.forEach(p=>{let l=p.Limits,t=p.Type,xy=t=="xy",po=t=="polar",aa=t=="ampang";if(xy||aa){x0=min(x0,l.Xmin);x1=max(x1,l.Xmax)};if(xy){y0=min(y0,l.Ymin);y1=max(y1,l.Ymax)};if(po||aa)r=max(r,l.Ymax)});
 ...
 let vline=(x,a)=>{let y0=a.ymin,y1=a.ymax,n=x.length,X=FA(3*n).fill(NaN),Y=FA(3*n).fill(NaN),j=0;x.forEach(x=>{X[j++]=x;X[j++]=x;Y[j-2]=y0;Y[j-1]=y1;j++});return[X,Y]}
 let hline=(y,a)=>{let x0=a.xmin,x1=a.xmax,n=y.length,X=FA(3*n).fill(NaN),Y=FA(3*n).fill(NaN),j=0;y.forEach(y=>{X[j++]=x0;X[j++]=x1;Y[j-2]=y;Y[j-1]=y;j++});return[X,Y]}
 ...
 let drawHilits=(a,p,f,t)=>{hi.filter(x=>x[0]==a.pi).forEach(h=>{let i=h[1],j=h[2];drawLine(a,p,p.Lines[i],i,f,t,j<0,1+j)})}
 ...
 let text=(x,y,s,a,f2,w,dx,dy)=>{dx=dx||0;dy=dy||0;if(!s)return;textalign(a);c.font=f2?font2:font1;let m=c.measureText(s,x,y),b={l:x-m.actualBoundingBoxLeft-2+dx,r:x+m.actualBoundingBoxRight+2+dx,t:y-m.actualBoundingBoxAscent-2+dy,b:y+m.actualBoundingBoxDescent+2+dy};if(w){let cl=c.fillStyle;c.fillStyle="white";c.fillRect(b.l+1-dx,b.t+1-dy,b.r-b.l-2,b.b-b.t-2);c.fillStyle=cl};c.fillText(s,x,y);return b}
 ...
 let drawPolar=(a,rt,unit)=>{let r=floor(a.w/2),cx=a.x+r,cy=a.y+r,R=(a.ymax-a.ymin)/2,xo=(a.xmax+a.xmin)/2,yo=(a.ymin+a.ymax)/2,o=(xo||yo)?1:0,r1=r+ticLength/2,r2=r-ticLength/2,r3=r+2*ticLength,al=[1,0,0,7,6,6,5,4,4,3,2,2],cs=cos(40*pi_),sn=sin(40*pi_);
  line(cx+r*cs,cy+r*sn,cx+r3*cs,cy+r3*sn);hitbox(hitPolimit,text(cx+r3*cs,cy+r3*sn,shortnum(R),6,0),a.pi,String(R));if(o)hitbox(hitPoloffs,text(cx+r3*cs,cy+r3*sn+o*fh1,"-"+sz(xo,yo),6),a.pi,"");text(cx+r3*cs,cy+r3*sn+(1+o)*fh1,""+unit,6);
 ...
 let plots=p=>{let g=grid(p.length,cols);dynstyle(min(g.w,g.h));c.reset();c.fillStyle="white";c.fillRect(0,0,w,h);rects=[];Axes=[];hits=[];check(p);limits(p);labels(p);p.forEach((p,i)=>{let[x,y]=xyi(g,i),pi=i+(single?single-1:0);rects.push({i:pi,x:x,y:y,w:g.w,h:g.h});c.save();c.translate(x+0.5,y+0.5);P[p.Type](p,pi,g.w,g.h);c.restore();});setslider(p)}
 
 let replot=_=>{cols=cnv.cols;w=cnv.width;h=cnv.height;plots(single?[p[single-1]]:p)},reset=_=>(single=0,hi=[],noanno(),reslimits(),replot())
 cnv.cols=cols;replot();cnv.replot=replot;

 if(cap&&tab){let c={},mpts=x=>{x=x.toSorted((a,b)=>a-b);x=abs(x[clamp(x.length>>>1,0,x.length-1)]);return x=x<0.01?5:x<0.1?4:x<1?3:x<10?2:x<100?1:0},k=Object.keys(tab);
  let fm=k.map(k=>"string"==tab[k][0]?-1:mpts(tab[k]));k.forEach((k,i)=>{let f=fm[i];c[k]=tab[k].map(x=>f<0?x:x.toFixed(f));let m=max(...c[k].map(x=>x.length));c[k]=c[k].map(x=>f<0?x.padEnd(m," "):x.padStart(m," "))});
  let fill=x=>{let n=max(...x.map(x=>x.length)),f=x=>x+" ".repeat(n-x.length);return x.map(f)},tr=(x,i)=>(i=x.indexOf("\\"))<0?x:x.slice(0,i)
  let icol=n=>fill(["#",...Array(n).fill(0).map((_,i)=>String(1+i))]) //todo custom numbers
  let q=Object.keys(c),t=q.map(x=>fill([tr(x),...c[x]]));t=[icol(t[0].length-1),...t];r=[]
  for(let i=0;i<t[0].length;i++)r.push(t.map(x=>x[i]).join("│"))
  let rowb="border-left:.5em solid black;padding-left:.2em;background:black;color:white;position:sticky;top:0;padding:0;padding-left:.2em"
  let rowi=i=>`border-left:.5em solid ${colors[i%colors.length]};padding:0;padding-left:.2em;`
  cap.innerHTML=r.map((s,i)=>`<option data-i="${i-1}" style="${i?rowi(i-1):rowb}">${hs(s).replaceAll(" ","&nbsp;")}</option>`).join("\n")
  cap.size=cap.children.length;cap.style.overflowY="hidden"
  cap.onchange=_=>{let n=p.length,i=af(cap.selectedOptions).map(x=>+(x.dataset.i)).filter(x=>x>=0);hi=[];iota(n).forEach(pi=>i.forEach(i=>hi.push([pi,i,-1])));replot()}}
 ...
 let contextmenu=e=>{pd(e);if(menu)menu.remove();let x=e.clientX+window.scrollX,y=e.clientY+window.scrollY,s=ce("select"),opt=(t,f)=>{let o=tc(t,ce("option"));o.onclick=e=>{s.remove();f()};ac(s,o)};opt("reset",reset);
 opt("zoom",_=>(pan=0,mea=0,cursor("zoom")));
 opt("pan",_=>(pan=1,mea=0,cursor("pan")));
 opt("measure",_=>(pan=0,mea=1,cursor("mea")));
 opt("copy png",copypng);s.size=s.childElementCount;s.style.cssText=`position:absolute;top:${y-5}px;left:${x-5}px;background=#ffe;border:1px solid;z-index:99;padding:0px;outline:none;overflow:hidden;font-family:monospace`;ac(document.body,s);s.value="";menu=s};cnv.addEventListener("contextmenu",contextmenu);
 cursor("zoom");
}
