...
function read8(a){return physread8(decode(a, false, curuser));}
function read16(a){return physread16(decode(a, false, curuser));}
function write8(a, v){return physwrite8(decode(a, true, curuser),v);}
function write16(a, v){return physwrite16(decode(a, true, curuser),v);}
...
function printstate() { cleardebug()
	writedebug("R0     R1     R2     R3     R4     R5     R6     R7\n"+
	 ostr(R[0],6)+" "+ostr(R[1],6)+" "+ostr(R[2],6)+" "+ostr(R[3],6)+" "+ 
	 ostr(R[4],6)+" "+ostr(R[5],6)+" "+ostr(R[6],6)+" "+ostr(R[7],6)+"\n\n[")
...
function interrupt(vec, pri) {
console.log("interrupt", vec, pri)
	var i;
...
function O(x){return "0"+x.toString(8)}

function step(){
	var val, val1, val2, ia, da, sa, d, s, l, r, o, max, maxp, msb;
printstate()
if(waiting)console.log("step: waiting")
	ips++;
	if(waiting) return;
	curPC = R[7];
	ia = decode(R[7], false, curuser);
console.log("step ia", O(ia), "pc", O(R[7]))
	R[7] += 2;
	lastPCs = lastPCs.slice(0,100);
	lastPCs.splice(0, 0, ia);
	instr = physread16(ia);
console.log("instr", O(instr))
	d = instr & 077;
	s = (instr & 07700) >> 6;
	l = 2 - (instr >> 15);
	o = instr & 0xFF;
...
function reset() {
	...
	for(i=0;i<K.length;i++)memory[01000+i] = K[i];
	...
	clkcounter = 0;
	waiting = false;
	writedebug("k loaded at 010000 ("+K.length+" words)")
}
function nsteps(n) {
	while(n--) {
		try {
