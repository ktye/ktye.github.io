...
func dyt(d node) byte {
	x, y, o := d.c[0].t, d.c[1].t, d.s[0]
	p := d.p
	rx, ry := rank(x), rank(y)
	mxr := rx
	if ry > mxr {
...
func token(b []byte) (tok func() node, pic func() byte) {
	const W = 11
	src = b
	s := make([]int, len(b))
	n := make([]node, 0)
