...
let parse=x=>{let[tok,pos]=x,locs,args,res,funs={},glob={},tabl={},data={},lp=0
 ...
 let avec=x=>(vt(x)?(lp=x.n,x.push("ioj","get "+temp("i","i"),...shift(x.t),"adi"),x.t=x.t.toLowerCase(),x):x)
 ...
 let take=(x,y,p,s,v)=>(p=" @"+p,tyck(x.t,"i",x.p),vt(y)?(y.push("j 32","slj","j 32","srl"),x.push("joi"+p,"j 32","slj","orj"),y.push(...x),y):(x.push(...y,(mark("alloc","fill."+y.t),"cal fill."+y.t)),x.t=y.t.toUpperCase(),x))
 let drup=(x,y,p,s,v)=>(p=" @"+p,tyck(x.t,"i",x.p),vt(y)?1:perr("type",y.p),y.push(...x,"joi"+p,"tee "+(n=temp("j","n.x")), "j "+("BCHSIEJFZZ".indexOf(y.t)/2|0),"slj","adj","get "+n,"j 32","slj","suj"+p),y)
...
*/
 ...
 let edat=(o,x,r)=>{r="\n";while(x.length){r+="\ndat "+o+" "+(x.length>8?x.slice(0,8):x).map(x=>x.toString(16).padStart(2,"0")).join(""); o+=8;x=x.slice(8)};return r}
