let M,rip,eop,asm
let RB=new Uint8Array(16*64),RH=new Uint16Array(RB.buffer),RU=new Uint32Array(RB.buffer),RK=new BigUint64Array(RB.buffer)
let loadelf=u=>{let U=new Uint32Array(u.buffer,0,u.buffer.byteLength>>>2),H=new Uint16Array(u.buffer,0,u.buffer.byteLength>>>1),n=H[28],p=U[8]>>>2,i,s=[],m=0
 eop=U[p+4]+U[p+8];for(i=0;i<n;i++){s.push({o:U[p+2],va:U[p+4],fs:U[p+8]});m=Math.max(m,U[p+4]+U[p+10]);p+=14};M=new Uint8Array(m);s.forEach(x=>M.set(u.subarray(x.o,x.o+x.fs),x.va));rip=U[6]}

let show=_=>{let s="";for(let i=0;i<10;i++)s+=M[rip+i].toString(16).padStart("0")+" ";return s}

let execv=a=>{
 let u=fsget(a[0]);if(u===0){progexit();return}
 try{loadelf(u)}
...
let D;
let disasm=(a,e)=>{asm=[];disa.innerHTML=""
 WebAssembly.instantiateStreaming(fetch("ud/dis.wasm")).then(r=>{D=r.instance.exports
  let ud=D.dis_init(a)
