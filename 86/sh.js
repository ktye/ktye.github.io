...
let rlprev=x=>{let n=rlbuf.length; console.log("rlprev",n,rlpos);  if(rlpos>0)rlpos--;rdl.textContent=(n?rlbuf[rlpos<n?rlpos[n]:""]:"");rlend()}
let rlnext=x=>{let n=rlbuf.length;  if(rlpos<n)rlpos++;rdl.textContent=(n?rlbuf[rlpos<n?rlpos[n]:""]:"");rlend()}

let prmt=x=>{prm.textContent=x?"█":"";rdl.focus()}
rdl.onkeydown=(e,k)=>(k=e.key,"Enter"==k?(pd(e),exec(rladd(rdl.textContent))):"ArrowUp"==k?(pd(e),rlprev(rdl.textContent)):"ArrowDown"==k?(pd(e),rlnext(rdl.textContent)):"Tab"==k?(pd(e),complete(rdl.textContent)):0)
...
let stdout=0,stdin=0
let exec=x=>{ out.textContent+=x+"\n";
 rdl.textContent="";x=x.trim()
 ...
 if(io>=0){ofile=a[io+1];a.splice(io,2);stdout=new Uint8Array(4096);stdout.n=0}
 if(x.length==0){}
 ...
 else{x=fsget(a[0]); console.log("xc??",x)
  if(x===0)out.textContent+="no file: "+a[0]+"\n";
  else if(String(af(x.slice(0,4)))!="127,69,76,70")out.textContent+="no elf: "+a[0]+"\n"
  else out.textContent+="todo: exec "+a[0]+"\n"
 }
 if(ofile!=""){fsadd(ofile,stdout.slice(0,stdout.n));stdout=0}
 shprmt()
} 
...
let rm=a=>{console.log("rm",...a);a.forEach(x=>fsdel(x))}
