...
func ini(mem []f) { // start function
	tab1 = [128]func(k) k{ // monads
		nil, nil, las, sqr, sin, cos, dev, log, exp, nil, abs, nil, rel, ima, phi, cnj, //  00- 15
		cnd, zxp, dia, avg, med, vri, prm, rnd, nil, nil, nil, nil, nil, nil, nil, nil, //  16- 31
	...
	tab2 = [128]func(k, k) k{ // dyads
		nil, nil, inn, wtn, bin, nil, nil, lgn, pow, nil, nil, nil, mkz, fns, rot, nil, //  00- 15
		nil, rxp, nil, mvg, pct, cov, nil, rol, nil, nil, nil, nil, nil, nil, nil, nil, //  16- 31
...
func mkk(s s, k s) { dec(asn(mks(s), mkb([]c(k)), 0)) } // store k implementation
...
func tip(x k) (r k) { // @x
	...
	if x == 0 {
		...
		return dex(x, c2s(mkc(byte('0'+t-V0))))
	} else if t == A && n != atom {
	...
	if v == 0 { // (;…) → list
		...
		if n > 3 && v < 256 {
			...
			for i := k(0); i < n-2; i++ { // last index may be a vector
				if m.k[2+i+r] == 0 || m.k[m.k[2+i+r]]&atom != atom {
					return R() + atm(v, r)
				}
			}
		}
...
*/
func strong(x k) bool {
	if x < 256 {
		if x > 127 {
			x -= 128
		}
		if (x >= '0' && x <= '9') || (x > 22 && x < 31) || (x > 32+22 && x < 64) {
			return false // io-verbs and built-ins >= rand
		}
		return true
	}
	if t, n := typ(x); t < V1 {
		...
		return strong(m.k[3+x])
	} else if n == 2 { // projection
		return strong(m.k[2+x])
	} else if n == 3 { // composition
		return strong(m.k[2+x]) && strong(m.k[2+x])
	}
	return false
}
func idm(x k) (r k) { // idiom-replacement
	cdr := m.k[3+x]
	ct, cn := typ(cdr)
	if ct == L && cn == 2 && m.k[2+x] == '*' && m.k[2+cdr] == '|' { // *|.. → last
		return l2(2, las(las(x)))
	}
	return x
}
func uneval(x k) (r k) { // uneval
	if t, n := typ(x); t == L && n > 0 {
		return cat(0, x)
	}
	return x
}
func evc(x k) (r k, full bool) { // partially evaluate
	t, n := typ(x)
	if t == S && n == atom {
		return x, false
	} else if t != V0 {
		return x, false // defer expr
	} else if t != L || n == 0 {
		return x, true
	} else if n == 1 {
		return fst(x), true
	...
	r, full = mk(L, n), true
	var f bool
	for i := k(0); i < n; i++ { // left-to-right is ok
		m.k[2+r+i], f = evc(inc(m.k[2+x+i]))
		if full && !f {
			full = false
			for j := k(0); j < i; j++ {
				m.k[2+r+j] = uneval(m.k[2+r+j])
			}
		}
		if !full && f {
			m.k[2+r+i] = uneval(m.k[2+r+i])
		}
	}
	dec(x)
	if v := m.k[2+r]; full && strong(v) && !(v == dy+'$' && n > 3) {
		if v == 0 {
			return uf(drop(1, r)), true // list
		}
		return cal(inc(v), uf(drop(1, r))), true
	}
	return r, false
}
func fld(x k) (r k) { // constant-folding
	if x, full := evc(x); full {
		return uneval(x)
	} else {
		return x
	}
	...
	if v < 128 && v != 0 && n == 3 { // assign(::;`x;v) modified(+:;`x;v)
		...
		if nt, nn := typ(name); nt == L && nn > 1 {
			if m.k[2+name] == 0 { // (;`a;`b) vector assignment
				...
				if nn := m.k[idx] & atom; nn == 1 {
					return dex(x, lcat(lcat(lcat(com(fst(idx), l), 'A'), nvar(name)), v))
				} else {
					return dex(x, lcat(lcat(lcat(com(cat(0, idx), l), 'B'), nvar(name)), v))
				}
			}
		}
		return dex(x, lcat(lcat(lcat(l, 'a'), nvar(name)), v))
	...
	for i := k(0); i < xn; i++ {
		...
		case xi == 'x':
			...
			if f > 255 && m.k[f]>>28 < V0 && ln > 1 { // matrix index
				for i := k(0); i < ln-1; i++ { // last index may be a vector
					if m.k[2+i+r] == 0 || m.k[m.k[2+i+r]]&atom != atom {
						push(atm(f, r))
						continue next
					}
...
func add(x, y k) (r k) { f, g := op2('+'); return nd(x, y, 0, f, g) } // x+y
func sub(x, y k) (r k) { f, g := op2('-'); return nd(x, y, 0, f, g) } // x-y
func mul(x, y k) (r k) { f, g := op2('*'); return nd(x, y, 0, f, g) } // x*y
func div(x, y k) (r k) { f, g := op2('%'); return nd(x, y, 0, f, g) } // x%y
func min(x, y k) (r k) { f, g := op2('&'); return nd(x, y, 0, f, g) } // x&y
func max(x, y k) (r k) { f, g := op2('|'); return nd(x, y, 0, f, g) } // x|y
...
func cat(x, y k) (r k) { // x,y
	...
	case xt == A:
		...
		if xn == atom && yn == atom { // d,d
			...
			return dex(y, amdv(x, inc(m.k[2+y]), 0, inc(m.k[3+y])))
		}
		if match(m.k[2+x], m.k[2+y]) == false {
...
func vsc(x, y k) (r k) { // `csv?y  x 0: y, ("ii";"|")0:("2|3";"3|4";"4|5")
	...
	p = amdv(p, wer(eql(mkc('z'), inc(p))), 0, mkc('f')) // @[p;&"z"=p;"f"]
	m.k[2+x] = p
	r = kxy(mks(".vsc"), x, y)
	nc := m.k[cc] & atom
	if nc == 0 {
...
func env(e, l, y, v k) (r k) { // call-with-env
	...
	for i := k(0); i < n; i++ {
		...
		if i < v {
			dec(asn(name, inc(m.k[2+y+i]), 0))
		} else {
			dec(asn(name, mk(I, 0), 0))
		}
	}
	dec(y)
	r = evl(inc(l))
...
func ltr(x k) (r k) { // `p@{..} (lambda tree, k7: .{...})
	...
	r = mk(L, 4)
	args, tree, v := m.k[2+m.k[3+x]], m.k[3+m.k[3+x]], t-V0
...
func ovr(f, x k) (r k) { // f/x
	if t, n := typ(f); t < V0 { // x/y (idiv, dot)
		if t == L || n != atom {
			return dot(f, x)
		} else if t > I {
			panic("type")
	...
	if ny := m.k[y] & atom; ny != atom {
		...
		for i := k(0); i < ny; i++ {
			if n := m.k[m.k[2+i+r]] & atom; n == 0 {
				...
				m.k[2+i+r] = amd(take(mx, 0, mki(0)), tak(mki(n-mx), jota(mx)), 0, m.k[2+i+r])
			}
		}
...
func inn(x, y k) (r k) { panic("nyi(in)") }     // x in y
func wtn(x, y k) (r k) { panic("nyi(within)") } // x within y
...
func sel(t, c, b, a k) (r k) { // #[t;c;b;a] select
	...
	dec(c)
	g := false
	if b == 0 || (m.k[b]>>28 == A && m.k[m.k[2+b]]&atom == 0) {
		dec(b)
	} else {
		d := k(0)
		t, d = gby(t, b)
...
func udt(t, c, b, a k) (r k) { // _[t;c;b;a] update
	...
	for i := k(0); i < nk; i++ {
		...
		m.k[2+j+cols] = amdv(m.k[2+j+cols], inc(c), 0, u)
	}
	dec(v)
	m.k[3+r] = cols
	return decr(t, c, r)
...
func dxn2(name, idx k) (k, k) { // `a.b[`c] → `a `b`c (todo: dnx2→dxn)
	c, n := sc(2 + name)
	for i := k(0); i < n; i++ {
		if m.c[c+i] == '.' {
			break
	...
	r := mk(L, m.k[x]&atom)
	for i := k(0); i < m.k[x]&atom; i++ {
		m.k[2+r+i] = enl(c2s(inc(m.k[2+i+x])))
	}
	return dex(x, name), cat(r, idx)
}
...
func dmdv(x, a, f, y k) (r k) { // dmd on value(x)
	...
	if n == atom {
		...
		return decr(x, a, r)
	} else if n == 2 && t == L && (m.k[m.k[2+a]]&atom != atom || m.k[2+a] == 0) { // matrix assign
		...
		for i := k(0); i < m.k[a0]&atom; i++ {
			...
			x = amdv(inc(x), inc(ai), 0, amdv(atx(x, ai), inc(a1), inc(f), yi))
		}
		return decr(f, decr(a0, a1, y), x)
	}
	a0 := fst(inc(a))
	return amdv(inc(x), inc(a0), 0, dmdv(atx(x, a0), drop(1, a), f, y))
}
func lup(x k) (r k) { // lookup
	if r = lupo(inc(x)); r == 0 || r == 0xffffffff {
		panic(undef(x))
	}
	return dex(x, r)
}
...
func nvar(x k) (r k) {
	...
	if i, o := varn(x); o {
		return dex(x, mki(k(i)))
	}
	m.k[kkey] = cat(m.k[kkey], x)
	m.k[kval] = lcat(m.k[kval], 0)
	n = m.k[m.k[kval]] & atom
	m.k[1+n+m.k[kval]] = 0xffffffff // (undef)
	return mki(n - 1)
}
...
func (p *p) noun() (r k) {
	...
	case p.t(sOcb):
		...
		ln := m.k[args] & atom
		r = mk(V1, 0) // lambda is indicated with length 0 but uses 2 fields:
		...
		lk := m.k[lkey]
		m.k[lkey] = m.k[2+l]
		m.k[4+l] = com(fld(inc(m.k[3+r])), mk(L, 0))
		m.k[lkey] = lk
		m.k[3+r] = l
		m.k[r] = (V0+ln)<<28 | 0
		return p.idxr(r)
...
func atox(x k) (r k) {
	...
	x = xeh(take(8, 0, cat(x, take(8-n, 0, mkc('0')))))
	r = mk(I, atom)
	m.k[2+r] = m.k[2+x]
	return dex(x, r)
}
