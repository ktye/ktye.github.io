...
//timestamps: seconds since fit-epoche(1989): 631065600(unix offset)
...
let parse=b=>{let i,j,r={time:[],dist:[],lat:[],lon:[],bpm:[],alt:[],lap:[]},v=new DataView(b),p=0,e=s=>{throw new Error(p+':'+s)}
 ...
 let p20=f=>{let s=180/2147483648,ti=0,la=0,lo=0,di=0,bp=0,al=0;f.forEach(x=>x.i==253?ti=U():x.i==0?la=s*I():x.i==1?lo=s*I():x.i==2?al=H()/5-500:x.i==3?bp=B():x.i==5?di=0.01*U():x.i==78?al=U()/5-500:p+=x.s);
