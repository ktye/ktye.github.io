...
static A th(w,wd,fmt)A w;I wd;void(*fmt)();{
 ...
 DO(m, DO(c, x+=dv[i]; k=strlen(y); MC(x-k-(c>1+i),MC(q,y,k),k); y+=wd;););
 EPILOG(z);
}
...
static F1(jpr1){C*v;I c,m=1,n,q,r,*s;
 RZ(w);
...
F1(jpr){PROLOG;I t;
 ...
 else if(t&VERB+ADV+CONJ){A sa,ta,y;I j;
  if(evoke(w)){sa=ta=VAV(w)->f; RZ(y=symbrd(sa));}else{sa=dash; ta=mtv; y=w;}
  DO(*qdisp, j=qdisp[1+i]; RZ(jpr1(thorn1(1==j?arep(y):2==j?drep(y):
      3==j?srep(sa,y):4==j?trep(ta,y):lrep(y)))););
 }
 EPILOG(w);
}
