...
/* ----------------------------------------------------------------------- */
/*                                                                         */
/* Parsing: Tacit Adverb/Conjunction Translator (s : 12)                   */

#include "j.h"
#include "p.h"


static A tbs,tdex,tial,tlev,ttie;
static B tcj;
...
static A tfork(a,fs,w)A a,fs,w;{A p,q;C c;V*u,*v;
 ...
 if(tcj){
  ...
  if(CONJ&AT(fs))R ADV&AT(a)&&veq(w,tial)?thook(a,fs):hooko(tial,forko(a,fs,w));
  if(veq(a,tial))R thook(fs,w);
  u=VAV(a); c=u->id;
...
TACT(cforkv){AA p,q,r,z;
 ...
 if(tcj)switch((p.t?4:0)+(q.t?2:0)+(r.t?1:0)){
  ...
  case 4: z.t=gt2(p.t,ltie(tie(q.a,r.a)));                  break;
  case 5: z.t=thook(tial,forko(thook(tlev,p.t),q.a,thook(tlev,r.t))); break;
  case 6: z.t=gt2(tfork(consa(p),ttie,consa(q)),ltie(r.a)); break;
  case 7: z.t=gt1(tfork(tfork(consa(p),ttie,consa(q)),ttie,consa(r)));
 }
 R z;
...
F1(ctrans){PROLOG;A loc=local,z;
 ...
 RZ(ttie=ds(CGRAVE));
 RZ(tbs =ds(CBSLASH));
 tcj=all1(eps(box(cstr("y.")),words(w)));
 GA(local,SYMB,twprimes[0],1,0);
 RZ(tname=link(ds(CALPHA),   tcj?ds(COMEGA):mtv));
 RZ(tval =link(tcj?tlev:tial,tcj?tdex      :mtv));
