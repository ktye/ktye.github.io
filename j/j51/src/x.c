...
static DF1(ri){A y,z;I n,r,*s,t;
 ...
 GA(z,t,n,r,0); MC(AS(z),s,AN(w)-AH*sizeof(I));
 R z;
}
...
static DF1(lrx){PREF1(lrx); ASSERT(BOX&AT(w),EVDOMAIN); R lrep(symbrd(onm(w)));}
...
static DF1(tss){R scf(tod()-tssbase);}

static DF2(tsit2){D t;I n,old;
 F2RANK(0,1,tsit2,0);
 RE(n=i0(a));
...
static DF1(tsit1){R tsit2(one,w,0L);}
...
#if (!LINKJ && SYS&SYS_SESM && SYS&SYS_PC+SYS_PC386)
...
static DF1(dispq){A z; GA(z,INT,*qdisp,1,0); ICPY(AV(z),1+qdisp,*qdisp); R z;}

static DF1(disps){I n;
 RZ(w=vi(w));
 n=AN(w);
 ASSERT(n,EVLENGTH);
 ASSERT(1>=AR(w),EVRANK);
...
static DF1(boxs){RZ(vs(w)); ASSERT(11==AN(w),EVLENGTH); MC(qbx,AV(w),11L); R mtv;}
...
#if (SYS & SYS_PC386)
  case XC(11,0):  R CDERIV(CIBEAM, jwin1,   jwin2,    RMAXL,RMAXL,RMAXL);
#endif
