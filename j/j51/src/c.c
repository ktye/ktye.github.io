...
static CS1(atcon1, f1(gs,fs))

static CS2(atcon2, f1(gs,fs))
...
F2(atop){
 RZ(a&&w);
 switch(CONJCASE(a,w)){
  case VN: R CDERIV(CAT,atcon1,atcon2,RMAXL,RMAXL,RMAXL);
  case VV: R CDERIV(CAT,on1,   upon2, mr(w),lr(w),rr(w));
...
F2(pdt){PROLOG;A z;I an,ar,ar1,*as,m,mn,n,p,t,wn,wr,wr1,*ws;
 ...
 switch(t){
  case BOOL:
   {B*u,*v,*wv;I*x,*zv;
    u=an+(B*)AV(a); wv=wn+(B*)AV(w); zv=mn+AV(z); memset(AV(z),C0,mn*sizeof(I));
