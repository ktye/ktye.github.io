...
F2(take){PROLOG;A x=qfill,z;B b;C*xv,*zv;I k,m,n,r,*s,t,*u,xn;
 ...
 if(b||!AN(w)){I*v;
  ...
  if(m)if(xn)mvc(k*m,zv,k,xv); else fillv(t,m,zv);
  ...
  for(i=0;i<n;++i){
   ...
   if(q!=m){
    ...
    if(q>m)if(xn)mvc(k*AN(y),yv,k,xv); else fillv(t,AN(y),yv);
