...
static F1(trr){PROLOG;A fs,gs,hs,t,*x,y,z;B p;C c,d,id;I m;V*v;
 RZ(w);
 ...
 if(id==CADVF&&(c=ID(fs),d=ID(gs),(p=c==CGRAVE)||d==CGRAVE||d==CATDOT||d==CGRCO)){
  y=p?gs:fs;
  RZ(x[!p]=tleaf(spellout(p?c:d)));
  RZ(x[ p]=NOUN&AT(y)?treach(every(y,fx)):trr(y));
 }else{
