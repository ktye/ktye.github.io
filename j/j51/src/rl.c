...
static F2(linsert){A f,g,h,*u,y;B ft,gt,ht,vb;C id,p,q,r;I n;V*v;   
 RZ(a&&w);
 ...
 switch(id){
  ...
  case CFORK:
   ...
   R raze(y);
  default:
   y=over(lcp(ft||NOUN&AT(v->f)&&id!=CGRAVE&&id!=CATDOT&&lp(f),f),lsymb(id));
...
F1(lrep){PROLOG;A fs,gs,hs,t,*tv,y,z;B p;C c,d,id;I m;V*v;
 RZ(w);
 ...
 if(id==CADVF&&(c=ID(fs),d=ID(gs),(p=c==CGRAVE)||d==CGRAVE||d==CATDOT||d==CGRCO)){
  y=p?gs:fs;
  RZ(tv[!p]=spellout(p?c:d));
  RZ(tv[ p]=NOUN&AT(y)?ltie(every(y,fx)):lrep(y));
 }else{
  RZ(tv[0]=(id==CATDOT||id==CGRCO)&&1>=AR(fs) ? ltie(hs) : lrep(fs) );
  if(1<m)RZ(tv[1]=lrep(gs));
