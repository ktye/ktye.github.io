...
let vmin=x=>Math.min(...x),vmax=x=>Math.max(...x),min=Math.min,max=Math.max,floor=Math.floor,ceil=Math.ceil,abs=Math.abs
...
function plots(p,canvas,slider,listbx,legend){const z=devicePixelRatio //zoom scale
 ...
 let endzoom=(_x,_y,X,Y,ri)=>{
  ...
  if(zoom.pan){if(zoom.hv){[dx,dy]=(abs(dx)>abs(dy))?[dx,0]:[0,dy];if(zoom.meas){zoom=null;legend(["dx","dy"][+(dy>dx)]+": "+(dx+dy));return}};
 ...
 canvas.onwheel=e=>{let[x,y,X,Y,ri]=mousecoords(e);if(e.deltaY==0)return;let z=(e.deltaY>0)?2:0.5,pi=p.plots[ri],l=pi.limits,xm=(l[0]+l[1])/2,ym=(l[2]+l[3])/2,DX=(l[1]-l[0])/2*z,DY=(l[3]-l[2])/2*z
  if(pi.type=="polar"){if(DX>abs(xm)&&DY>abs(ym)){xm=0;ym=0};l[0]=xm-DX;l[1]=xm+DX;l[2]=ym-DY;l[3]=ym+DY}else{let xo=(X-xm)*z,yo=(Y-ym)*z;if(X>l[0]){l[0]=X-xo-DX;l[1]=X-xo+DX};if(Y>l[2]){l[2]=Y-yo-DY;l[3]=Y-yo+DY}}
  replot(canvas)}
}
