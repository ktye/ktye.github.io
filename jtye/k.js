...
scan  =f=>x=>x.length?(x.reduce((a,b,i)=>x[i]=i?f(b,a):a,x[0]),x):x,
...
list  =x=>[x],
string=x=>(typeof x==="string")?JSON.stringify(x):(atom(x)||typeof x==="function")?String(x):x.every(x=>!isNaN(x))?x.map(String).join(" "):"("+x.map(string).join(";")+")",
