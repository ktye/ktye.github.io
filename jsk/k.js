...
mul   =atomic((x,y)=>x*y),
div   =atomic((x,y)=>x/y),
mod   =atomic((x,y)=>x?(y%=x,y+x*+(y<0)):y),
idiv  =atomic((x,y)=>x?~~((y-(x-1)*+(y<0))/x):y),
min   =atomic(Math.min),
max   =atomic(Math.max),
less  =atomic((x,y)=>+(x<y)),
more  =atomic((x,y)=>+(x>y)),
...
match =(x,y)=>(type(x)!=type(y))?0:(atom(x)?+(x==y):(x.length!=y.length)?0:+x.every((x,i)=>match(x,y[i]))),
dict  =(x,y)=>x.reduce((r,x,i)=>(r[x]=y[i],r),{}),
at    =(x,y)=>(typeof x==="function")?x(y):atom(x)?x:atom(y)?x[y]:right(at)(x,y),
find  =(x,y)=>atom(y)?(-1<(y=x.indexOf(y))?y:x.length):right(find)(x,y),
cut   =(x,y)=>atom(x)?(x<0?cut(Math.floor(y.length/-x),y):cut(mul(Math.ceil(y.length/x),til(x)),y)):[...x].map((x,i,a)=>y.slice(x,a[i<a.length-1?1+i:a.length])),
take  =(x,y)=>atom(x)?at(y,x<0?add(y.length+x,til(-x)):til(x)):x.filter(x=>y.includes(x)),
drop  =(x,y)=>atom(x)?(x<0?y.slice(0,x):y.slice(x)):x.filter(x=>!y.includes(x)),
cat   =(x,y)=>atom(x)?cat(list(x),y):x.concat(y),
split =(x,y)=>y.split(x),
join  =(x,y)=>y.join(x),
dec   =(x,y)=>y.slice(1).reduce((r,y,i)=>y+mul(at(x,i),r),first(y)),
enc   =(x,y)=>rev(rev(x).map(x=>{let t=mod(x,y);y=idiv(x,y);return t}))
