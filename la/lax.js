...
dffffffffffffffffffffffffffblcddiffffkfffffffffffffffffffffbdcda
aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa`.replaceAll("\n","")).map(x=>x-97)
let T=ic(`
abcdefghijfdkbabcdefghijfdkbabcdefghidfddbabcdrfghijfddbabcdefgh
ijfddbabcdemmhidmddbabcdennhidoddbppppppprpppqppabcdemghidmddbab
cdefnhijfddblllllllllllllblllllllllllllbabcdemmhidmddbabcdennhid
oddbabcdennhinnddbppppppprpppqppppppppppppppppabcdefghidfddb`.replaceAll("\n","")).map(x=>x-97)

let token=(x,s,i,l)=>(s=0,i=ic(x).map(x=>s=T[14*s+C[x]]).map((x,i)=>x>10?-1:i).filter(x=>x>=0),
 x=i.map((_,k)=>x.slice(i[k],i[1+k])),
 s=i.map((_,j)=>((x[j]==" ")||((j==0||x[j-1]==" "||x[j-1]=="\n")&&(x[j][0]=="/"&&x[j][1]!="'"))?0:1)),
